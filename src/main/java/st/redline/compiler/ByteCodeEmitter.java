...
class ByteCodeEmitter implements Emitter, Opcodes {
    ...
    private void openPrivateSendMessagesMethod() {
        // This method sends all the messages encompassed in your smalltalk source file.
        if (isTraceEnabled(LOG))
            LOG.trace("");
        mv = cw.visitMethod(ACC_PRIVATE, "sendMessages", SEND_MESSAGES_SIG, null, null);
        mv.visitCode();
    ...
    public void closeClass(boolean returnRequired) {
        if (isTraceEnabled(LOG))
            LOG.trace(source.fullClassName());
        closePublicSendMessagesMethod(returnRequired);
        cw.visitEnd();
    ...
    private void closePublicSendMessagesMethod(boolean returnRequired) {
        if (isTraceEnabled(LOG))
            LOG.trace("");
        if (returnRequired)
            mv.visitInsn(ARETURN);
